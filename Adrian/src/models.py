...
class Classifier(torch.nn.Module):
    def __init__(self, args, out_features=2, in_features=None):
        ...
        self.traditional_classifier = RandomForestClassifier(n_estimators=100, random_state=42)
        self.activation = torch.nn.ReLU()

        if in_features is not None:
            num_feats = in_features
        elif args.experiment_type in ['sp_lstm_A_trainer', 'sp_lstm_B_trainer', 'sp_weighted_lstm_A', 'sp_weighted_lstm_B']:
        ...
        self.mlp = torch.nn.Sequential(
            ...
            self.activation,
            torch.nn.Linear(in_features=args.gcn_parameters['cls_feats'], out_features=out_features)
        )
    ...
    def forward(self, x, y=None):
        ...
        # If training, fit the traditional classifier
        # * ONLINE LEARNING: fit the traditional classifier with the new data
        if self.training and y is not None:
            y_np = y.detach().cpu().numpy()
            self.traditional_classifier.fit(x_np, y_np)

        # Use the traditional classifier to predict
        predictions_np = self.traditional_classifier.predict(x_np)

        # Convert predictions back to torch tensor
        predictions = torch.tensor(predictions_np, dtype=torch.float32).to(x.device)
